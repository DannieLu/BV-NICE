    ...
    def __init__(self, x, t, y_ , p, FLAGS, r_alpha, r_lambda, do_in, do_out, dims):
        ...
        for i in range(0,n_in):
            if i==0:
                ...
                weights_in.append(tf.Variable(tf.random_normal([dim_in,dim_rep], stddev=weight_init)))
            else:
            ...
            if FLAGS.varsel and i==0:
                ...
                biases_in.append(tf.Variable(tf.random_normal([1,dim_rep], stddev=weight_init)))
        ...
        h_out = [tf.concat([h_rep,t],axis=1)]
        for i in range(0,n_out):
            if i==0:
                # weights_out.append(tf.Variable(tf.random_normal([dim_in+1,dim_out], stddev=weight_init)))
                weights_out.append(tf.Variable(tf.random_normal([dim_rep+1,dim_out], stddev=weight_init)))
            else:
