...
if VAL:
    ...
    t1_ind_val = T_val[:,1]==1   # find which column has the treatment == 1
    t0_ind_val = T_val[:,0]==1


    n0_val = np.sum(t0_ind_val)
    n1_val = np.sum(t1_ind_val)


    X0_val = X_val[t0_ind_val]
    X1_val = X_val[t1_ind_val]

    Y0_val = Y_val[t0_ind_val]
    Y1_val = Y_val[t1_ind_val]
...
results = [folder_ind, file_ind ,pehe_ls,pehe_val_ls,pehe_test_ls,y_std]
